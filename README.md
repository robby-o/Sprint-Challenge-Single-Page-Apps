...
Client-side routing is in comparison to server-side routing. Server-side routing necessitates a page reload whenever a link, new data, or a new UI is requested by the user. So, client-side routing prevents that.

The inital rendering of the app downloads all the information that the user would need to navigate throughout the app, and whenever a link to within a location in the app, or new data or UI is requested, the user does not see a page reload nor is a network request made. The app loads seamlessly and quickly as the user navigates around.
